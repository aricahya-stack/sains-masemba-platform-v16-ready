...
async function serialize(tryoutId: string, viewerId = '') {
  const item = await prisma.tryout.findUniqueOrThrow({
    where: { id: tryoutId },
    include: {
      author: { select: { id: true, fullName: true, role: true } },
      questions: { include: { question: { include: { blueprint: true } } }, orderBy: { orderNo: 'asc' } },
    },
  });
  ...
  return {
    ...
    _persisted: 'true',
    _readOnly: 'false',
    _deleteDisabled: item.authorId === viewerId ? 'false' : 'true',
    scheduleOwner: `Dibuat oleh ${item.author.fullName}${item.author.role === UserRole.SUPER_ADMIN ? ' (Super Admin)' : ' (Guru)'}`,
    tryoutCode,
...
async function resolveQuestions(
  ...
  if (missing.length) {
    throw new Error(`Soal tryout berikut tidak ditemukan dalam bank soal bersama: ${missing.slice(0, 5).join(', ')}${missing.length > 5 ? ', ...' : ''}`);
  }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const questions = await resolveQuestions(String(body.questionCodes || ''), requireThirty, String(body.tryoutCode || body.sourceGroup || ''));
    ...
    return NextResponse.json({ data: await serialize(item.id, user.id) });
  } catch (error) {
...
export async function PUT(request: Request) {
  ...
  const target = await prisma.tryout.findFirst({
    where: {
      id: tryoutId,
      author: { is: { role: { in: [UserRole.GURU, UserRole.SUPER_ADMIN] } } },
    },
  });
  if (!target) return NextResponse.json({ error: 'Tryout tidak ditemukan.' }, { status: 404 });
  ...
  try {
    ...
    return NextResponse.json({ data: await serialize(tryoutId, user.id) });
  } catch (error) {
...
export async function DELETE(request: Request) {
  ...
  if (!owned) return NextResponse.json({ error: 'Tryout tidak ditemukan.' }, { status: 404 });
  if (owned.authorId !== user.id) {
    return NextResponse.json({ error: 'Jadwal dapat diedit bersama, tetapi hanya pembuat jadwal yang dapat menghapusnya.' }, { status: 403 });
  }
