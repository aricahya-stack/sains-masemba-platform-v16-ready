...
export default async function NilaiTryoutAdminPage({ searchParams }: { searchParams: Promise<{ kelas?: string }> }) {
  ...
  return (
    <div className="stack">
      <PageHero
        eyebrow="Ujian • Nilai Tryout"
        title="Rekap nilai tryout per kelas"
        description="Pilih kelas untuk melihat nilai setiap tryout yang sudah dikerjakan. Jika siswa mengerjakan tryout berulang kali, seluruh nilainya ditampilkan berurutan dalam satu kolom, misalnya 30; 45; 65."
      />
        ...
        <section className="card stack">
          <div className="section-title-row">
            <div>
              ...
              <strong>{selectedStudents.length} siswa • {tryouts.length} tryout dikerjakan</strong>
            </div>
            ...
            <table className="data-table class-report-table tryout-score-table">
              ...
              <tbody>
                {selectedStudents.length ? selectedStudents.map((student, index) => (
                  ...
                  <tr><td colSpan={Math.max(tryouts.length + 2, 2)}><div className="empty-state">Tidak ada siswa pada kelas ini.</div></td></tr>
                )}
