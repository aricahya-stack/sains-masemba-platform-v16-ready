...
async function resolveAuthorId(value: unknown, fallbackId: string) {
  ...
  if (!author) throw new Error('Pemilik soal tidak valid.');
  return author.id;
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const question = await prisma.$transaction(async (tx) => {
      ...
      const created = await tx.question.create({ data: { ...buildQuestionData(body), authorId } });
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.question.update({ where: { id: questionId }, data: { ...buildQuestionData(body), authorId } });
