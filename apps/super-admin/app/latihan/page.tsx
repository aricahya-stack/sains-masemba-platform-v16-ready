...
export default async function LatihanAdminPage() {
  ...
  const fields: InlineFieldDef[] = [
    ...
    { name: 'authorId', label: 'Pemilik soal', type: 'select', options: authors.map((author) => ({ value: author.id, label: `${author.fullName} • ${author.role === UserRole.GURU ? 'Guru' : 'Super Admin'}` })) },
  ...
  return (
    <InlineEditableManager
      ...
      newRowDefaults={{ authorId: admin.id, status: 'DRAFT', questionType: 'SINGLE_CHOICE', scoringMode: 'EXACT_MATCH', maxScore: '1', stimulusOrder: '1', blueprintId: '' }}
